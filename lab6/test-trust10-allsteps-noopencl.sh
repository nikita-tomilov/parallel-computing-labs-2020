...
run() {
  ...
  for N in $(seq 1200 3380 35000); do
    for K in $(seq 1 10); do
      ./lab6 $N $M 200 --noopencl | grep ':' | tee -a logs-$M-$K.txt
      sleep 1
    done
  done
}
...
results_allsteps_total() {
  for M in $(echo 0 1 2 3 4 5 6 12); do
    for K in $(seq 1 10); do
    ...
    pr -W 256 -t -m *.txta | tee results-noopencl-allsteps-threads-$M.csv
    rm -rf *.txta
  done
}
...
results_step() {
  ...
  for M in $(echo 0 1 2 3 4 5 6 12); do
    ...
    pr -W 256 -t -m *.txta | tee results-noopencl-step-"$STEP"-threads-$M.csv
    rm -rf *.txta
  done
}
...
mkdir -p "results_trust10_noopencl"
mv results-*.csv ./results_trust10_noopencl/
